    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            ...
            variant === "floating" || variant === "inset"
             ...
             variant === 'floating' || variant === 'inset' ? 'group-data-[state=expanded]:w-[var(--sidebar-width)]' : '',
             className
          )}
        />
        <div
        ...
        >
          <div
            data-sidebar="sidebar"
            className={cn("flex h-full w-full flex-col bg-sidebar", // Main sidebar container styling
                         variant === "floating" ? "rounded-lg border border-sidebar-border shadow" : "", // Apply rounding only for floating
                         variant === "inset" ? "rounded-lg" : "" // Apply rounding for inset
                        )}
          >
            {children}
          </div>
    ...
    return (
      <Tooltip>
        ...
        <TooltipContent
          ...
          className="rounded-md" // Use rounded-md for tooltip
          {...tooltip}
        />
      </Tooltip>
    )
...
>(({ className, ...props }, ref) => (
  <div
    ...
    className={cn(
      "absolute right-1 flex h-5 min-w-5 items-center justify-center rounded-md px-1 text-xs font-medium tabular-nums text-sidebar-foreground select-none pointer-events-none", // Use rounded-md
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return (
    <div
      ...
      className={cn("rounded-md h-8 flex gap-2 px-2 items-center", className)} // Use rounded-md
      {...props}
    >
      {showIcon && (
        <Skeleton
          className="size-4 rounded-md" // Use rounded-md
          data-sidebar="menu-skeleton-icon"
        />
      )}
      <Skeleton
        className="h-4 flex-1 max-w-[--skeleton-width] rounded-md" // Use rounded-md
